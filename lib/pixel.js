 ...
 * igennem betalingen.
 */
export function spor(navn, data, eventID = null) {
  try {
    if (!PIXEL_ID || typeof window === "undefined") return;
    if (!maa("marketing")) return;
    if (typeof window.fbq !== "function") return;
    // ⚠️ TREDJE ARGUMENT, IKKE ET FELT I `data` (22-09-2026). Meta laeser
    // event_id fra options-objektet; lagde vi det i parametrene, ville det blive
    // sendt som en almindelig custom-parameter og deduplikeringen ville aldrig
    // traede i kraft — uden en fejl nogen steder.
    //
    // ⚠️ MANGLER ID'ET, SENDER VI ALLIGEVEL. En haendelse uden event_id er en
    // haendelse der ikke kan deduplikeres mod serveren; en haendelse der aldrig
    // blev sendt, er en tabt konvertering. Vi vaelger den foerste.
    if (eventID) window.fbq("track", navn, data || {}, { eventID });
    else window.fbq("track", navn, data || {});
...
export function sporEnGang(noegle, navn, data, eventID = null) {
