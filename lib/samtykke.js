...
import { maalSamtykke } from "./samtykkeMaaling";
...
export const KATEGORIER = [
  ...
  // ⚠️ "FUNKTIONELLE" ER FJERNET (15-09-2026, Jonas). Den stod her som GB-only
  // og blev lovet i BEGGE cookiepolitikker - men ingen kode laeste nogensinde
  // maa("funktionel"), og der blev aldrig sat en funktionel cookie. Maalt, ikke
  // formodet: nul brugssteder i hele repoet.
  //
  // ⚠️ SKAL DEN TILBAGE, KOMMER DEN SAMMEN MED KODEN DER SAETTER COOKIEN -
  // ikke foer. Det var praecis den raekkefoelge der skabte problemet: teksten
  // blev skrevet til et produkt vi troede vi ville bygge.
  //
  // ⚠️ INGEN VERSION-BUMP. Et bump ville bede hver eneste besoegende om samtykke
  // igen, fordi en kategori FORSVANDT. Et gammelt valg der naevner "funktionel"
  // er harmloest: maa() slaar op i det gemte objekt, og en kategori der ikke
  // laenger findes, bliver aldrig spurgt om.
  {
    id: "statistik",
    navn: "Statistik",
...
export function gemSamtykke(valg) {
  ...
  // ⚠️ MÅLINGEN FØR UDSENDELSEN, OG MED VILJE. `birdly-samtykke` tømmer køen i
  // ctaSporing.js; havde samtykke-hændelsen ligget efter, ville den lande efter
  // de hændelser den er forudsætningen for, og rækkefølgen i tragten ville
  // fortælle at kunden landede før hun sagde ja.
  maalSamtykke(fuldt, "valgt");
...
export function nulstilSamtykke() {
  ...
  ryddMarketing();
  // ⚠️ EN TILBAGEKALDELSE ER OGSÅ ET VALG. Uden den her ville en kunde der
  // trak sit ja tilbage se ud som om hun aldrig havde taget stilling — og
  // afvisningsraten ville se bedre ud end den er.
  maalSamtykke(INTET, "nulstillet");
