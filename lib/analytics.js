...
export function tilPostHog(event, step, attribution = {}, props = {}) {
  try {
    ...
    // ⚠️ `phProps`, IKKE `props` - OG DET ER IKKE KOSMETIK.
    // Parameteren hedder `props` (husets egne event-props). Hed den lokale
    // ogsaa `props`, ville `...(typeof props.antal ...)` inde i initialiseringen
    // referere til den const der er ved at blive oprettet - en TDZ-fejl:
    // "Cannot access 'props' before initialization". Den blev slugt af
    // try/catch'en nedenfor, og resultatet var at HVER ENESTE PostHog-haendelse
    // forsvandt tavst. Maalt i produktion: 9 kald til husets eget lag, 0 til
    // PostHog. Fail-open goer et vaerktoej ufarligt - og en fejl usynlig.
    const phProps = {
    ...
    if (!ph) {
      if (PH_KOE.length < PH_KOE_MAKS) PH_KOE.push({ navn, props: phProps, tid: new Date().toISOString() });
      return;
    }
    ph.capture(navn, phProps);
  } catch { /* måling må aldrig vælte siden */ }
}
