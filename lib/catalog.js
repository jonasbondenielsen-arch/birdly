...
// Opret en Frisbii embedded subscription-session (server-side, priv-nøgle aldrig her).
// Returnerer { ok, session_id } til montering i Reepay.EmbeddedSubscription. Frisbii
// ejer al betalingslogik; vi sender kun kunden ind. Payload: { subscriber_id, email,
// contact_name, phone, billing }.
export async function createSubscriptionSession(payload) {
  const res = await fetch(fnUrl("create-subscription-session"), {
    method: "POST",
    headers: {
      apikey: ANON || "",
      Authorization: `Bearer ${ANON || ""}`,
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok || !body.session_id) throw new Error(body.error || "Kunne ikke starte betalingen (" + res.status + ").");
  return body;
}
