...
// ---------- TEASER (engangs re-engagement) ----------
//
// ⚠️ EGEN TOKEN-TYPE. Et teaser_token er IKKE et share_token og maa aldrig bruges i
// hinandens sted. Det peger paa ét tilbud til ét firma paa én opgave, og det doer
// efter 24 timer.
//
// ⚠️ DER KOMMER ALDRIG KONTAKTOPLYSNINGER RETUR HERFRA. Ikke "endnu ikke" — feltet
// findes ikke i svaret overhovedet. Kontakten aabnes foerst naar virksomheden BAADE
// har taget en plads og er tilmeldt igen, og den hentes ad den almindelige vej.
// Bygger du en visning her der leder efter `kontakt`, leder du efter noget der ikke
// kan komme.

export const hentTeaser = (token) => kald({ action: "teaser", token });

// Tager en af de tre pladser. Aabner IKKE kontakten — den returnerer virksomhedens
// eget fortsaet-link, saa hun kan vaelge plan.
export const tagTeaser = (token) => kald({ action: "teaser_tag", token });
