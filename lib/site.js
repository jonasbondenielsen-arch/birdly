...
export function hreflangFor(path = "/") {
  ...
  for (const m of Object.values(MARKEDER)) {
    // ⚠️ `lanceret`, IKKE `serverbar` — og skiftet fra `aktiv` er ikke kosmetik.
    // GB kan serveres på en preview længe før det er i luften. Filtrerede vi på
    // "kan serveres", ville hvert dansk sidevisning straks udsende et hreflang
    // mod getbirdly.co.uk — et domæne der endnu ikke svarer. Et hreflang mod
    // en død adresse er ikke neutralt: Google kan miste tilliden til HELE
    // gruppen, altså også birdly.dk.
    if (m.lanceret !== true) continue;
