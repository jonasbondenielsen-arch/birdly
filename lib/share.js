...
// Valgfri grund til at en opgave blev fjernet. Kaldes EFTER fjernelsen og må aldrig
// blokere den — fejler den, beholder kunden stadig sin ryddede liste.
export async function sendDismissReason(token, matchId, grund) {
  try {
    const res = await fetch(fnUrl("save-my-criteria"), {
      method: "POST",
      headers: { ...AUTH(), "Content-Type": "application/json" },
      body: JSON.stringify({ action: "grund", token, match_id: matchId, grund }),
    });
    const b = await res.json().catch(() => ({}));
    return !!b.ok;
  } catch {
