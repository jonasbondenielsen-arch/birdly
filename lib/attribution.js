...
export function fangAttribution() {
  ...
  // ⚠️ HELE OBJEKTET BAG MARKETING-SAMTYKKET (15-09-2026, Jonas).
  //
  // Her var kun MARKETING_FELTER gated, mens UTM'erne blev gemt uanset. Det var
  // forsvarligt læst isoleret — en utm_source er ikke persondata i sig selv —
  // men objektet er ét: landing, tidspunkt og kampagnenavn tegner tilsammen et
  // besøg, og det er attribution uanset hvilket felt der bærer id'et.
  //
  // Afviser kunden marketing, gemmes der ingenting. Hun kan stadig tilmelde
  // sig; vi ved bare ikke hvilken annonce hun kom fra.
  if (!maa("marketing")) return gemt;
  for (const f of UTM_FELTER) saet(f, q.get(f));
  for (const f of MARKETING_FELTER) saet(f, q.get(f));
